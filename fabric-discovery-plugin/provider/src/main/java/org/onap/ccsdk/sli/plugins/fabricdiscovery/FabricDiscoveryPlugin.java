...
public class FabricDiscoveryPlugin implements SvcLogicJavaPlugin, IFabricDiscoveryService {
    ...
    @Override
    public void processDcNotificationStream (Map<String, String> paramMap, SvcLogicContext ctx) {
        try {
            ...
            if (enable) {
                LOG.info("Start monitoring notification stream: " + stream);
            } else {
                LOG.info("Stop monitoring  notification stream: " + stream);
            }
                    ...
                    try {
                        ...
                        if (enable) {
                            if (streamMap.get(stream) != null) {
                                LOG.info("Notification Stream: " + stream + " is already being monitoried");
                                return;
                            }
            ...
            LOG.info("Parameter Parsing exisited with exeception: ", e.getMessage());
        }
    }
